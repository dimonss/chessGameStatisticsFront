...
export function GameCard({ game, onClick }: GameCardProps) {
  ...
  const opponentName = opponent?.name || t('common.loading');
  const resultIcons = {
      ...
      <div className="relative p-6">
        <div className="flex items-start justify-between mb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <span className={`inline-block px-4 py-1.5 rounded-full text-sm font-bold ${resultBadges[game.result]} shadow-md`}>
                {t(`game.${game.result}`)}
              </span>
            </div>
          </div>
        ...
        <div className="mb-4">
          ...
          <div className="flex items-center gap-3 mt-3 text-sm text-gray-600">
            <span className="flex items-center gap-1.5 px-3 py-1.5 bg-white/70 rounded-lg backdrop-blur-sm font-medium">
              ...
              <span>{t(`game.${game.color}`)}</span>
            </span>
            <span className="flex items-center gap-1.5 px-3 py-1.5 bg-white/70 rounded-lg backdrop-blur-sm font-medium">
              <Clock className="w-3.5 h-3.5" />
              <span>{t(`game.timeControls.${game.timeControl}`)}</span>
            </span>
            <span className="flex items-center gap-1.5 px-3 py-1.5 bg-white/70 rounded-lg backdrop-blur-sm font-medium">
        ...
        <div className="flex items-center justify-between pt-4 border-t border-white/50">
          <div className="flex items-baseline gap-2">
            <span className="text-xs font-medium text-gray-500 uppercase tracking-wide">{t('player.rating')}</span>
        ...
        {game.opening && (
          <div className="mt-4 pt-4 border-t border-white/50">
            <div className="flex items-center gap-2">
              <span className="text-xs font-semibold text-gray-500 uppercase tracking-wide">{t('game.opening')}</span>
